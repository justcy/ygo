...
func NewYLog(prefix string, flag int) *YLoggerCore {
	...
	zlog := &YLoggerCore{prefix: prefix, flag: flag, isolationLevel: 0, calldDepth: 2}
	...
	runtime.SetFinalizer(zlog, CleanYLog)
	return zlog
}
...
*/
func (log *YLoggerCore) formatHeader(t time.Time, file string, line int, level int) {
...
func (log *YLoggerCore) verifyLogIsolation(logLevel int) bool {
...
// (关闭日志绑定的文件)
func (log *YLoggerCore) closeFile() {
	if log.fw != nil {
		log.fw.Close()
	}
}
